...
#define KMOBIMChildPath @"Chat/File"


@implementation MOBIMFileManager
...
+ (NSString *)filePathWithName:(NSString *)fileKey
                       orgName:(NSString *)name
                          type:(NSString *)type
{
    NSString *path = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:KMOBIMChildPath];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL isDirExist = [fileManager fileExistsAtPath:path];
    if (!isDirExist) {
        BOOL isCreatDir = [fileManager createDirectoryAtPath:path withIntermediateDirectories:YES attributes:nil error:nil];
    ...
    if (fileKey == nil) {
        return [path stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.%@",name,type]];
    }
    return [path stringByAppendingPathComponent:[NSString stringWithFormat:@"%@_%@.%@",fileKey,name,type]];
}

+ (NSString *)fileLocalPathWithURLString:(NSString*)URLString
{
    NSString *path = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:KMOBIMChildPath];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    BOOL isDirExist = [fileManager fileExistsAtPath:path];
    ...
    return [path stringByAppendingPathComponent:[NSString stringWithFormat:@"%@%@",[NSString md5:URLString],URLString.lastPathComponent]];
                ...
                toPath:(NSString *)toPath
{
    ...
    if (error) {
        //MOBIMLog(@"copy file error:%@",error);
    }
    return result;
}
